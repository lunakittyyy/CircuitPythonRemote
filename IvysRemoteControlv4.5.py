...
# Set up consumer_control for use in media keys
consumer_control = ConsumerControl(usb_hid.devices)
# Version tuple to prevent crashes later
def versiontuple(v):
    return tuple(map(int, (v.split("."))))
# Define an IR pin var for later
IR_PIN = board.IR_RX

# All IR arrays. Replace these with your own IR timings as you'd like.
...
# Set up the IR reciever and decoder
pulses = pulseio.PulseIn(IR_PIN, maxlen=200, idle_state=True)
decoder = adafruit_irremote.GenericDecode()
pulses.clear()
pulses.resume()
# Actions for each button press, with a loop comparing detected IR signals with all of the arrays
while True:
    ...
    if fuzzy_pulse_compare(mute, detected):
        ...
        # Use the version tuple defined at line 13 and 14 to see if adafruit_hid version is high enough to use brightness functions
        # If adafruit_hid is version 5.0.1 or later, change the brightness
        ...
        # Otherwise, do nothing and print a warning message in the console
        elif versiontuple(adafruit_hid.__version__) < versiontuple("5.0.1"):
            print('Current adafruit_hid version does not appear to support brightness, update to at least 5.0.1')
    elif fuzzy_pulse_compare(brightnessdown, detected):
        print('Turning brightness down')
        # Same comments from line 68 and 69 apply
        if versiontuple(adafruit_hid.__version__) >= versiontuple("5.0.1"):
            consumer_control.send(ConsumerControlCode.BRIGHTNESS_DECREMENT)
        # Same comment from line 72 applies
        elif versiontuple(adafruit_hid.__version__) < versiontuple("5.0.1"):
            print('Current adafruit_hid version does not appear to support brightness, update to at least 5.0.1')
